...
RC_OBJ = $(BUILD_DIR)/Selycord_rc.o
...
all: $(BUILD_DIR)/Selycord $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/Selycord_rc.o: Selycord.rc assets/Selycord.ico
	mkdir -p $(BUILD_DIR)
	windres Selycord.rc -o $@

$(BUILD_DIR)/%.o: %.c
	mkdir -p $(dir $@)
	$(CC) -c -o $@ $(FLAGS) $< -Wall -Wno-unused-function -Wno-misleading-indentation $(OPTS)

$(BUILD_DIR)/Selycord: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/Selycord.desktop
	rm -f $(PREFIX)/share/pixmaps/Selycord.png
	rm -f $(PREFIX)/share/pixmaps/Selycord.svg
	rm -f $(PREFIX)/bin/Selycord

install: uninstall
	cp Selycord.desktop $(PREFIX)/share/applications/Selycord.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/Selycord.svg
	cp $(BUILD_DIR)/Selycord $(PREFIX)/bin/Selycord

run: all
	cp assets/Selycord.gschema.xml $(BUILD_DIR)/assets/Selycord.gschema.xml
