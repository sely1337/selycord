...
// ── CRITIQUE : userData = dossier Selycord pour les settings/plugins
...
const SelycordData = path.join(app.getPath("appData"), "Selycord");
app.setPath("userData", SelycordData);
...
// pour le token Discord stable. Quand Selycord charge ce cache, il lit des permissions
...
// AppUserModelId unique — Windows reconnaît Selycord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour Selycord sont bloqués.
        ...
        app.once("browser-window-created", () => {
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[Selycord] Module bloqué (inutile pour Selycord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[Selycord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[Selycord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[Selycord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(SelycordData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[Selycord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[Selycord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[Selycord] LevelDB check failed:", e.message); }

// Modules bundlés dans Selycord-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[Selycord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[Selycord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans Selycord-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[Selycord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
console.log("[Selycord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir) && !buildInfo.localModulesRoot) {
        ...
        console.log("[Selycord] build_info.json patché → localModulesRoot:", nativeModulesDir);
    }
} catch (e) {
    console.warn("[Selycord] Impossible de patcher build_info.json:", e.message);
}
