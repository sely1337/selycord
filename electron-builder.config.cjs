...
// ─── Configuration de Build Selycord ─────────────────────────────────────────

function killSelycord() {
    const releaseDir = join(__dirname, "release", "Selycord-dist");
...
function buildSelycordFromDiscord(discordApp) {
    ...
    console.log("[Selycord] Préparation de _app.asar...");
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "Selycord", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const SelycordPreload = join(__dirname, "selycord-preload.js");
    if (existsSync(SelycordPreload)) {
        cpSync(SelycordPreload, join(outDist, "preload.js"));
    }
    ...
    const iconSrc = join(__dirname, "selycord.ico");
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "Selycord" --set-version-string "FileDescription" "Selycord"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[Selycord] Build terminé -> ${outDir}`);
}
...
buildSelycordFromDiscord(discord);

module.exports = {
    appId: "com.Selycord.app",
    productName: "Selycord",
    copyright: "Copyright 2026 Selycord",
