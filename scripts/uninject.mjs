/*
 * Selycord — Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection Selycord présente
...
function uninject(resourcesDir) {
    ...
    // Vérifier que le dossier app/ a bien été créé par Selycord
    if (existsSync(appDirPath)) {
        try {
            const pkg = JSON.parse(readFileSync(join(appDirPath, "package.json"), "utf-8"));
            if (pkg.name !== "Selycord") {
                console.warn(`\x1b[33m[Selycord] Le dossier app/ existe mais n'a pas été créé par Selycord (name: "${pkg.name}").\x1b[0m`);
                console.warn("\x1b[33m            Abandon pour éviter de casser un autre mod.\x1b[0m");
                return false;
            }
        } catch { }

        console.log("[Selycord] Suppression du dossier app/ injecté...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[Selycord] Aucun dossier app/ injecté trouvé.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[Selycord] Restauration _app.asar → app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[Selycord] app.asar déjà présent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[Selycord] Désinjection réussie depuis : ${resourcesDir}\x1b[0m`);
...
if (allResources.length === 0) {
    console.error("\x1b[31m[Selycord] Aucune installation Discord avec Selycord injecté trouvée.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que Selycord a bien été injecté via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}
...
if (uninjectCount === 0) {
    console.error("\x1b[31m[Selycord] Aucune désinjection réussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[Selycord] ${uninjectCount}/${allResources.length} désinjection(s) réussie(s).\x1b[0m`);
