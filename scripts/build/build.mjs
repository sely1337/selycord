...
const globNativesPlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-natives" }, async () => {
            const pluginDirs = ["plugins", "userplugins", "selycordplugins"];
            let code = "";
            let natives = "\n";
            let i = 0;
            /**
...
const buildConfigs = ([
    ...
    },
    {
        ...commonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/Vencord.ts")],
        outfile: "dist/selycord/renderer.js",
        format: "iife",
        target: ["esnext"],
    ...
    },
    {
        ...nodeCommonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/preload.ts")],
        outfile: "dist/selycord/preload.js",
        footer: { js: "//# sourceURL=file:///VencordPreload\n" + sourceMapFooter("preload") },
        sourcemap,
        define: {
            ...defines,
...
await Promise.all([
    ...
    writeFile("dist/selycord/package.json", JSON.stringify({
        name: "selycord",
        main: "main.js"
    }))
]);
...
await Promise.all([
    ...
    createPackage("dist/selycord", "dist/selycord.asar"),
]);
