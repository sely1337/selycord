...
function createSelycordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Selycord",
            submenu: [
                {
                    label: "About Selycord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Selycord" : "Check for Updates",
                    click: () => sendToRenderer(IpcEvents.TRAY_CHECK_UPDATES)
                },
                {
                    label: "Repair Selycord",
...
export function patchTrayMenu(): void {
    // No longer patching tray menu — Selycord manages its own tray in patcher.ts
}
