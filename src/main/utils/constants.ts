...
export const DATA_DIR = process.env.SELYCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "SelycordData", suffix)
        : join(app.getPath("userData"), "..", "Selycord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Selycord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
