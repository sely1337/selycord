...
import { platformName } from "@selycordplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Selycord developers"
        const SelycordGroupIds = [
        ...
        const isSelycordGroup = SelycordGroupIds.includes(userId);

        if (!isOriginalDev && !isSelycordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Selycord developers";
