...
let SelycordBadges = {} as Record<string, Array<{ icon: string; placeholder: string; uuid: string; }>>;
...
async function loadAllBadges(noCache = false) {
    ...
    const selycordData = await loadBadges(`https://api.${domain}/badges`, noCache).catch(() => ({}));

    DonorBadges = vencordBadges;
    EquicordDonorBadges = equicordBadges;
    SelycordBadges = selycordData;
}
...
export default definePlugin({
    ...
    getSelycordBadges(userId: string) {
        try {
            const userBadges = SelycordBadges[userId];
            ...
            return userBadges
                ...
                .map(badge => ({
                    iconSrc: badge.icon,
                    description: badge.placeholder ?? "Selycord Badge",
                    position: BadgePosition.START,
                    props: {
                        style: {
                            borderRadius: "0%",
            ...
            console.error("[BadgeAPI] Error processing Selycord badges for", userId, e);
            return [];
        }
    }
});
