...
import { t, useTranslation } from "../autoTranslateSelycord";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("Selycord_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("Selycord_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Selycord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `Selycord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("Selycord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    },
    stop() {
        removeHeaderBarButton("Selycord-event-logs");
