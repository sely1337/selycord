...
import { t } from "../autoTranslateSelycord";
...
const MI_TOKEN_CACHE_KEY = "Selycord-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("Selycord-mi-ctx-root") ?? document.createElement("div");
        el.id = "Selycord-mi-ctx-root";
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("Selycord-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("Selycord-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("Selycord-mi-ctx-root");
        if (root) root.remove();
    },
});
