...
function createTokenPreload(token: string): string {
    ...
    const dir = join(app.getPath("userData"), "Selycord-mi-preloads");
    mkdirSync(dir, { recursive: true });

    const safeToken = JSON.stringify(token); // échappe proprement le token

    const script = `
// Selycord MultiInstance — token preload
// S'exécute dans le main world AVANT Discord
(function() {
    const TOKEN = ${safeToken};
    try {
        // Définit le token dans localStorage
        Object.defineProperty(window, '__Selycord_token', { value: TOKEN, writable: false });
        ...
        console.log("[SelycordMI] Token preload active ✓");
    } catch(e) {
        console.warn("[SelycordMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const uniqueAppId = `Selycord.instance.${userId}.${Date.now()}`;
        ...
        const partition = `persist:Selycord-mi-${userId}`;
        ...
        const win = new BrowserWindow({
            ...
            title: `Selycord [${username || userId}]`,
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `Selycord [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[SelycordMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__SELYCORD_GW_WS__;
...
// ─────────────────────────────────────────────────────────────────────────────
// Fenetres « groupées » — meme groupe que Selycord dans la barre des taches
// Principe : on ne touche PAS a setAppDetails => la fenetre herite de l'AppId
// du processus principal (com.Selycord.app), Windows la groupe automatiquement
// ─────────────────────────────────────────────────────────────────────────────
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[SelycordMI] arrangeSplit error:", e);
    }
}
