...
const SELYCORD_RELEASES_REPO = "Selycord/Selycord";
const SELYCORD_REPO_URL = `https://git.${domain}/${SELYCORD_RELEASES_REPO}`;
...
export { SELYCORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Selycord, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(SELYCORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
