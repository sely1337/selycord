...
function useTutorialExists(pluginName: string) {
    ...
    React.useEffect(() => {
        ...
        let cancelled = false;
        fetch(
            `https://git.${domain}/Selycord-tutorials/raw/branch/main/videos/${pluginName}.mp4`,
            { method: "HEAD" }
        )
            .then(res => {
                if (!cancelled) {
                    ...
                    window.dispatchEvent(new CustomEvent("Selycord-tutorial-detected", { detail: { pluginName, found } }));
                }
            })
            .catch(() => {
                if (!cancelled) {
...
export function PluginCard({ plugin, disabled, onRestartNeeded, onMouseEnter, onMouseLeave, isNew }: PluginCardProps) {
    const globalSettings = useSettings(["language"]);
    const lang = globalSettings.language ?? "en";
    const localizedDescription = lang === "tr" && plugin.descriptionTr ? plugin.descriptionTr : plugin.description;
    ...
    const openTutorialVideo = (e: React.MouseEvent) => {
        e.stopPropagation();
        const videoUrl = `https://git.${domain}/Selycord/Selycord-tutorials/raw/branch/main/videos/${plugin.name}.mp4`;
        openModal(props => (
