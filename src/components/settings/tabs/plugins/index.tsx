...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Selycord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.SELYCORD });
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("Selycord-tutorial-detected", handler);
        return () => window.removeEventListener("Selycord-tutorial-detected", handler);
    }, []);
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.SELYCORD:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/selycordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (PluginMeta[plugin.name].folderName.startsWith("src/selycordplugins/") || PluginMeta[plugin.name].folderName.startsWith("src/plugins/_")) return false;
    ...
    const { SelycordPlugins, othersPlugins, requiredPlugins } = useMemo(() => {
        const SelycordPlugins = [] as JSX.Element[];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for Selycord to function."
                ...
                const isSelycord = folderName.startsWith("src/selycordplugins/");
                const card = (
                    <PluginCard
                        onRestartNeeded={handleRestartNeeded}
                        disabled={false}
                ...
                if (isSelycord) {
                    SelycordPlugins.push(card);
                } else {
                    othersPlugins.push(card);
                }
            }
        }
        return { SelycordPlugins, othersPlugins, requiredPlugins };
    ...
    const allPlugins = [...SelycordPlugins, ...othersPlugins];
    ...
    const SelycordVisible = SelycordPlugins.slice(0, Math.min(visibleCount, SelycordPlugins.length));
    const othersVisible = othersPlugins.slice(0, Math.max(0, visibleCount - SelycordPlugins.length));
        ...
        <SettingsTab>
                    ...
                    <ErrorBoundary noop>
                        <Select
                            options={[
                                ...
                                { label: "Show Selycord Plugins", value: SearchStatus.SELYCORD },
            ...
            {premiumOnly ? (
                <>
                    ...
                    {SelycordPlugins.length || othersPlugins.length
                        ? (
                            <div className={cl("grid")}>
                                {[...SelycordVisible, ...othersVisible].length
                                    ? [...SelycordVisible, ...othersVisible]
                ...
                <>
                    {SelycordPlugins.length > 0 && (
                        <>
                            <HeadingTertiary className={Margins.top20}>Selycord Plugins</HeadingTertiary>
                            <div className={cl("grid")}>
                                {SelycordVisible}
                            </div>
                        </>
                    ...
                    {SelycordPlugins.length === 0 && othersPlugins.length === 0 && (
                        <ExcludedPluginsList search={search} />
                    )}
