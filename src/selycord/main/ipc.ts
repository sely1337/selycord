...
handleSync(IpcEvents.GET_GIT_HASH, () => Selycord_GIT_HASH);
...
// Handler pour VencordNative.Selycord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.SelycordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { SelycordDir } = State.store;
    if (!SelycordDir) return;

    const stats = await stat(SelycordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(SelycordDir);
});
...
handle(IpcEvents.SELECT_VENCORD_DIR, async (_e, value?: null) => {
    if (value === null) {
        delete State.store.SelycordDir;
        return "ok";
    }
